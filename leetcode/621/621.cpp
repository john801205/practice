...
class Solution
{
  public:
    ...
    {
      std::vector<char>::size_type                 total = 0;
      std::array<std::vector<char>::size_type, 26> counts = {0};
      ...
      std::priority_queue<std::vector<char>::size_type> unused;
      std::queue<std::vector<char>::size_type>          cooling;

      for (const auto &count: counts)
        if (count != 0)
          unused.emplace(count);
      ...
      while (total > 0)
      {
        ...
        if (cooling.front() == 0 && unused.empty())
        {
          cooling.pop();
          cooling.emplace(0);
        }
        ...
        {
          auto remaining = cooling.front();
          cooling.pop();

          if (remaining != 0)
            unused.emplace(remaining);

          remaining = unused.top()-1;
          unused.pop();

          cooling.emplace(remaining);
          total--;
        }
      }
